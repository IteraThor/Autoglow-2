document.addEventListener('DOMContentLoaded', () => {
    ...
    function isDeviceActive(device) {
        ...
        return device.connection_type === 'serial' 
    ...
    async function checkAutodartsAuthStatus() {
        ...
        try {
            ...
            if (data.connected) {
                ...
                if (loadedConfig.autodarts_online_enabled) {
                    loadedConfig.autodarts_online_enabled = false;
                    if (onlineBoardApiToggle) {
                        onlineBoardApiToggle.checked = false;
                    }
                    updateOnlineStateLocks();
    ...
    async function linkAutodartsAccount() {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                loadedConfig.autodarts_online_enabled = true;
                if (onlineBoardApiToggle) {
                    onlineBoardApiToggle.checked = true;
                }
                ...
                try {
                    await fetch('/api/config', {
                        ...
                        body: JSON.stringify({ autodarts_online_enabled: true })
                    });
                } catch (err) {
                    console.error('Failed to save online enabled config:', err);
                }
                ...
                if (data.status === 'success') {
                    ...
                    try {
                        await fetch('/api/config', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ autodarts_online_enabled: false })
                        });
                    } catch (err) {
                        console.error('Failed to save online disabled config:', err);
                    }
...
<div class="inline-settings-container" style="display: flex; flex-direction: column; gap: 16px; width: 100%; margin-top: 0;">
            ...
            <span class="spinner" style="width: 10px; height: 10px; border-width: 2px;"></span>
                    ...
                    btnSave.addEventListener('click', async () => {
                        ...
                        try {
                            ...
                            let hasLocal = false;
                            segments.forEach(s => {
                                if (s.api_source === 'online' || s.api_source === 'hybrid') hasOnline = true;
                                if (s.api_source === 'local' || s.api_source === 'hybrid') hasLocal = true;
                                if (s.is_split && s.sub_segments) {
                                    s.sub_segments.forEach(sub => {
                                        if (sub.api_source === 'online' || sub.api_source === 'hybrid') hasOnline = true;
                                        if (sub.api_source === 'local' || sub.api_source === 'hybrid') hasLocal = true;
                                    });
                                }
                            });
                            ...
                            try {
                                ...
                                console.error('Failed to update device api_type in DB:', err);
                            }
    ...
    async function configureDevice(device, navigate = true) {
        ...
        if (segments.length === 0) {
            hasOnline = device.api_type === 'online' || device.api_type === 'hybrid';
            hasLocal = device.api_type === 'local' || device.api_type === 'hybrid';
        }
        ...
        const payload = {
            ...
            autodarts_online_enabled: hasOnline && isLinked,
            autodarts_websocket_enabled: hasLocal,
